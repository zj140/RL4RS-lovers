...
class Model(object):
    def __init__(self, item_vec, item_price, item_loc, item_max = 400, sess_max = 10, hist_max = 22):
    ...
    def build(self):
        self.hist = tf.placeholder(tf.int32, [None, self.sess_max, self.hist_max])
        self.hist_len = tf.placeholder(tf.int32, [None, self.sess_max])
        self.sess_num = tf.placeholder(tf.int32, [None])
        ...
        hist_emb = tf.nn.embedding_lookup(self.item_emb, self.hist) # [B, sess_num, hist_max, 64]
        hist_emb = tf.reshape(hist_emb, [-1, self.hist_max, 64])
        hist_len = tf.reshape(self.hist_len, [-1])
        # intra session representation
        sess_cell = tf.nn.rnn_cell.GRUCell(num_units=self.gru_size, name = 'intra_sess_gru')
        outputs, last_states = tf.nn.dynamic_rnn(
            cell=sess_cell, dtype=tf.float32, sequence_length=hist_len, inputs=hist_emb)
        sess_rep = tf.reshape(last_states, [-1, self.sess_max, self.gru_size])
        # inter session representation
        seq_cell = tf.nn.rnn_cell.GRUCell(num_units=self.gru_size, name = 'inter_sess_gru')
        outputs, last_states = tf.nn.dynamic_rnn(
            cell=seq_cell, dtype=tf.float32, sequence_length=self.sess_num, inputs=sess_rep)
        # target attention --> hist rep
        att = tf.matmul(outputs, tf.expand_dims(rec_avg, -1)) # [B, sess_max, 1]
        seq_mask = tf.reshape(
            tf.sequence_mask(self.sess_num, self.sess_max, dtype = tf.float32), [-1, self.sess_max, 1])
        att = att * seq_mask + (seq_mask-1) * (1e10)
        att = tf.nn.softmax(att, axis = 1)
        hist_rep = tf.reduce_sum(outputs * att, axis = 1) # [B, gru_size]
