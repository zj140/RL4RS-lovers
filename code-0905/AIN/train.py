...
def get_feed_dict(data, training = True, max_len = 60):
    ...
    for d in data:
        if len(d[0]) > max_len:
            hist.append(d[0][-max_len:])
            hist_len.append(max_len)
        else:
            hist.append(d[0] + [0] * (max_len-len(d[0])))
            hist_len.append(len(d[0]))
        item_rec.append(d[1])
        user_feat.append(d[2])
        label.append(d[3])
        listLabel.append(d[4])
    return {model.item_rec: item_rec, model.hist: hist, model.hist_len: hist_len, \
...
if __name__ == '__main__':
    ...
    model = Model(item_vec=item_vec, item_price=item_price, item_loc=item_loc, item_max = 400, hist_max = 60)
    model.build()
    ...
    for epoch in range(epoch_num):
        ...
        while start < train_size:
            ...
            _, step, loss, acc = sess.run([train_op, global_step, model.loss, model.list_acc], feed_dict)
            if (step+1) % 500 == 0:
                val_acc, val_acc_all, val_list_acc = evaluate(sess, val)
