...
def load_data(filename, user_feat_list = None):
    ...
    while True:
        try:
            ...
            for tmp in hist:
                i,t = tmp.split(':')
                hist_item.append(int(i))
                hist_time.append(int(t))
            ...
            time = int(temp[5])
            length.append(len(hist_item))
            data.append([hist_item, items, uFeat_id, labels, listLabel])
        except:
            break
    f.close()
    print('Total number of data', len(length))
    print(np.percentile(length, range(0,101,10)))
    for i in range(10):
